...
fn process_task(pending_task: &PendingTask) {
    ...
    let workdir = rtqd_work_dir()
        .join("tasks")
        .join(today)
        .join(format!("task_{}", pending_task.id));
    fs::create_dir_all(&workdir).expect(&format!(
...
fn run_command(task_command: &TaskCommand, workdir: &Path) -> std::io::Result<Child> {
    ...
    info!(
        "starting command. workdir{} command line {:?} ",
        workdir.to_str().unwrap(),
        cmds
    );
...
fn main() {
    ...
    if !log_file_dir.exists() {
        fs::create_dir_all(&log_file_dir).expect(&format!(
            "failed to create rtqd work dir: {}",
            log_file_dir.to_str().unwrap()
        ));
    }
