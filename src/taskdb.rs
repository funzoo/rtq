...
impl FinishedTask {
    pub fn new(working_task:&WorkingTask, time_finished:&str, exit_reason:&str, exit_code: i64) -> FinishedTask {
        FinishedTask {
            id : working_task.id,
...
impl TaskDB {
    pub fn new () -> Option<TaskDB> {
        ...
        if ! db_file_dir.exists() {
            fs::create_dir_all(&db_file_dir).expect(&format!("faild to create dir: {:?}", db_file_dir));
        }

        let db_file_path = db_file_dir.join("rtq.db");

        let conn = Connection::open(&db_file_path).expect(&format!("failed to open db file: {:?}", db_file_path));
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS working_tasks (
                      id              INTEGER PRIMARY KEY,
                      command         TEXT NOT NULL,
                      max_run_sec     INTEGER,
                      time_created    TEXT NOT NULL,
    ...
    pub fn load_task(&self) -> Option<PendingTask> {
       let mut stmt = self.conn.prepare("SELECT id, command, max_run_sec, time_created FROM pending_tasks").expect("failed to query db");
    ...
    fn load_working_tasks(&self) -> Vec<WorkingTask> {
       let mut stmt = self.conn.prepare("SELECT id, command, max_run_sec, time_created, time_started FROM working_tasks").expect("failed to query db");
    ...
    pub fn remove_pending_task(&self, pending_task:&PendingTask) {
        let mut stmt = self.conn.prepare("DELETE FROM pending_tasks where id= :id").unwrap();
        stmt.execute_named(&[(":id", &pending_task.id)]).unwrap_or_default();

    }

    pub fn remove_working_task(&self, working_task:&WorkingTask) {
        let mut stmt = self.conn.prepare("DELETE FROM working_tasks where id= :id").unwrap();
        stmt.execute_named(&[(":id", &working_task.id)]).unwrap_or_default();

    }

    pub fn insert_pending_task(&self, pending_task:&PendingTask) {
        let mut stmt = self.conn.prepare(
            "INSERT INTO pending_tasks values (:id, :command, :max_run_sec, :time_created)").unwrap();
        stmt.execute_named(&[
                           (":id", &pending_task.id),
    ...
    pub fn insert_working_task(&self, working_task:&WorkingTask) {
        let mut stmt = self.conn.prepare(
            ...
            :time_created, :time_started)").unwrap();
    ...
    pub fn insert_finished_task(&self, finished_task:&FinishedTask) {
        ...
        stmt.execute_named(&[
                           ...
                           (":exit_code", &finished_task.exit_code),]).unwrap();
    }
}
